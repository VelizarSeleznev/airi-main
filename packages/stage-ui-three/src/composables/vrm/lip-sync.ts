...
export function useVRMLipSync(audioNode: Ref<AudioBufferSourceNode | undefined, AudioBufferSourceNode | undefined>) {
  ...
  const fallbackAnalyser = audioContext.createAnalyser()
  fallbackAnalyser.fftSize = 256
  let fallbackBuffer = new Uint8Array(fallbackAnalyser.frequencyBinCount)
  const { state: lipSyncNode, isReady } = useAsyncState(async () => {
    if (!audioContext.audioWorklet) {
      console.warn('[VRM] Phoneme lip sync disabled: AudioWorklet is unavailable in this browser context. Falling back to amplitude lip sync.')
      return undefined
    }
    ...
    try {
      ...
      console.warn('[VRM] Phoneme lip sync disabled, falling back to amplitude lip sync:', error)
      return undefined
    }
  }, undefined)
  ...
  watch([isReady, audioNode], ([ready, newAudioNode]) => {
    if (!ready || !newAudioNode || lipSyncNode.value)
      return
    try {
      newAudioNode.connect(fallbackAnalyser)
    }
    catch {}
  }, { immediate: true })
  onUnmounted(() => audioNode.value?.disconnect())

  function getFallbackMouthWeight() {
    fallbackBuffer = fallbackBuffer.length === fallbackAnalyser.frequencyBinCount
      ? fallbackBuffer
      : new Uint8Array(fallbackAnalyser.frequencyBinCount)
    fallbackAnalyser.getByteTimeDomainData(fallbackBuffer)

    let sum = 0
    for (const value of fallbackBuffer) {
      const centered = (value - 128) / 128
      sum += centered * centered
    }

    const rms = Math.sqrt(sum / fallbackBuffer.length)
    return Math.max(0, Math.min(CAP, (rms - 0.015) * 5))
  }
  ...
  function update(vrm?: VRMCore, delta = 0.016) {
    ...
    if (!node) {
      const targetA = getFallbackMouthWeight()
      for (const key of LIP_KEYS) {
        const from = smoothState[key]
        const to = key === 'A' ? targetA : 0
