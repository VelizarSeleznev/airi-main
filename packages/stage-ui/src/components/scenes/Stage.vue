...
const props = withDefaults(defineProps<{
  ...
  scale?: number
  chatSpeechMode?: 'stream' | 'final-only' | 'off'
}>(), {
  paused: false,
  scale: 1,
  chatSpeechMode: 'stream',
})
...
let audioAnalyserBuffer: Uint8Array<ArrayBuffer> | undefined
...
function startLipSyncLoop() {
  ...
  const tick = () => {
    if (!nowSpeaking.value) {
      ...
      mouthOpenSize.value = getAnalyserMouthOpen()
    }
...
function syncLipSyncLoop() {
  ...
  }) && (lipSyncStarted.value || audioAnalyser.value)) {
    startLipSyncLoop()
    return
  }
...
async function setupLipSync() {
  ...
  if (!audioContext.audioWorklet) {
    ...
    setupAnalyser()
    startLipSyncLoop()
    console.warn('[Live2D] Lip sync disabled: AudioWorklet is unavailable in this browser context.')
    return
  }
...
function setupAnalyser() {
  if (!audioAnalyser.value) {
    ...
    audioAnalyserBuffer = new Uint8Array(audioAnalyser.value.frequencyBinCount)
  }
}

function getAnalyserMouthOpen() {
  const analyser = audioAnalyser.value
  if (!analyser)
    return 0

  audioAnalyserBuffer ??= new Uint8Array(analyser.frequencyBinCount)
  analyser.getByteTimeDomainData(audioAnalyserBuffer)

  let sum = 0
  for (const value of audioAnalyserBuffer) {
    const centered = (value - 128) / 128
    sum += centered * centered
  }

  const rms = Math.sqrt(sum / audioAnalyserBuffer.length)
  const normalized = Math.max(0, Math.min(1, (rms - 0.015) * 7))
  return normalized * 100
}

let currentChatIntent: ReturnType<typeof speechRuntimeStore.openIntent> | null = null

async function resetAssistantPresentation(reason: string) {
  ...
  if (currentChatIntent) {
    currentChatIntent.cancel(reason)
    currentChatIntent = null
  }
}

function openAssistantIntent() {
...
async function speakAssistantResponse(text: string) {
  ...
  try {
    ...
    if (currentChatIntent === intent)
      currentChatIntent = null
  }
}

chatHookCleanups.push(onBeforeMessageComposed(async () => {
  await resetAssistantPresentation('new-message')

  if (props.chatSpeechMode === 'stream')
    openAssistantIntent()
}))
...
else if (props.chatSpeechMode === 'final-only') {
  chatHookCleanups.push(onAssistantResponseEnd(async (message) => {
    await speakAssistantResponse(message)
  }))
}
